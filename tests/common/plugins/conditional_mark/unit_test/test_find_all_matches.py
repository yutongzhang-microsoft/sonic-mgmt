...
def test_true_conditions_in_longest_entry():
    ...
    for match in matches:
        for mark_name, mark_details in list(list(match.values())[0].items()):
            ...
            if mark_name == "skip":
                assert mark_details.get("reason") == "Skip test_conditional_mark.py::test_mark"

    assert len(marks_found) == 1 and 'skip' in marks_found
...
def test_partly_false_conditions_in_longest_entry():
    conditions, session_mock = load_test_conditions()
    ...
    for match in matches:
        for mark_name, mark_details in list(list(match.values())[0].items()):
            ...
            if mark_name == "xfail":
                assert mark_details.get("reason") == "Xfail test_conditional_mark.py::test_mark_1"
            elif mark_name == "skip":
                assert mark_details.get("reason") == "Skip test_conditional_mark.py::test_mark"

    assert len(marks_found) == 2 and 'skip' in marks_found and 'xfail' in marks_found


# Test case 3: All conditions in the matching path are false
def test_all_false_conditions_in_matching_path():
    conditions, session_mock = load_test_conditions()
